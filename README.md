# SQL para analistas de Ciberseguridad - X horas

## A QUIÉN ESTÁ DIRIGIDO:
Este curso está diseñado para principiantes y personas que deseen reforzar sus conocimientos en la creación de consultas SQL para extraer y manipular datos de bases de datos relacionales.

## TEMARIO:

### MÓDULO 0: [Introducción a BBDD y SQL (45min)](https://github.com/mauricioge/sql-queries-cs/blob/main/modulo0.md)
- **¿Qué es una Base de Datos Relacional?**
- **¿Qué es SQL y para qué sirve?**
- **Entorno de Trabajo**

### MÓDULO 1: [Consultas Básicas de Selección (1h 15min)](https://github.com/mauricioge/sql-queries-cs/blob/main/modulo1.md)
- **Sentencias `SELECT` y `FROM`**
- **Filtrar Resultados con `WHERE`**
- **Sentencia `INSERT INTO`**
- **Sentencia `UPDATE`**
- **Sentencia `DELETE`**
- **Ordenar Resultados (`ORDER BY`)**
- **Limitar Resultados (`LIMIT`/`TOP`)**
- **Orden lógico de ejecución de una consulta**

### MÓDULO 2: [Filtrado avanzado y manejo de NULL (40 min)](https://github.com/mauricioge/sql-queries-cs/blob/main/modulo2.md)
- **Operadores lógicos: `AND`, `OR`, `NOT`**
- **`BETWEEN`, `IN`, `LIKE` (con `%` y `_`)**
- **Manejo de `NULL`: `IS NULL`/`IS NOT NULL` (por qué `= NULL` no funciona)**
- **Funciones de reemplazo de `NULL`: `COALESCE`, `ISNULL` (SQL Server) / `NVL` (Oracle) / `IFNULL` (MariaDB)**

### MÓDULO 3: [Funciones de agregación y agrupamiento (40 min)](https://github.com/mauricioge/sql-queries-cs/blob/main/modulo3.md)
- **Funciones de agregación: `COUNT`, `SUM`, `AVG`, `MIN`, `MAX`**
- **Agrupar por una o varias columnas: `GROUP BY`**
- **`HAVING` vs `WHERE`**
- **Errores comunes**
- **Práctica (10 min)**

### MÓDULO 4: [JOINs (50 min)](https://github.com/mauricioge/sql-queries-cs/blob/main/modulo4.md)
- **Relaciones entre tablas**
- **Concepto de JOINs**
- **Tipos de JOINs más comunes**
- **Práctica + Preguntas y Respuestas (15min)**

### MÓDULO 5: [Subconsultas y CTEs(35 min)](https://github.com/mauricioge/sql-queries-cs/blob/main/modulo5.md)
- **Subconsultas (Subqueries)**
- **Introducción a CTEs**
- **Práctica (10 min)**

### MÓDULO 6: [Funciones escalares y manejo de fechas/texto (30 min)](https://github.com/mauricioge/sql-queries-cs/blob/main/modulo6.md)
- **Funciones de texto**
- **Funciones de fecha**
- **Conversión de tipos**
- **Práctica (5 min)**

## Consideraciones Adicionales para el Curso:
- **Práctica Constante:** Cada módulo incluye ejercicios prácticos para que los participantes puedan aplicar lo aprendido.
- **Base de Datos de Ejemplo:** Para los ejercicios que realizaremos utilizaremos una base de datos sencilla pero consistente pero con todo lo necesario para que entender los conceptos y realizar prácticas.
- **Demostraciones en Video:** Puedes ver esta clase en video acá.
- **Recursos Adicionales:**
